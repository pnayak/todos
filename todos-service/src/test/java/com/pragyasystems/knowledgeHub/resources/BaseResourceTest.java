...
public abstract class BaseResourceTest<T extends Entity> {
	...
	protected T entity;
	protected User user = new User("511846484800016990f86233");
	...
	public void addNewEntity() {
		...
		final Response response = resource.add(user, null, entity);
		...
		try {
			negResponse = resource.add(user, null, null);
	...
	public void updateEntity() {
		...
		when(mockRepository.findOne("511846484800016990f86233")).thenReturn(entity);
		when(mockRepository.save(entity)).thenReturn(entity);

		final Response response = resource.update(user, "511846484800016990f86233", null,
				entity);
		...
		Response negResponse;
		try {
			negResponse = resource.update(user, "511846484800016990f86233", null, null);
			fail("Should have thrown a WebApplicationException but did not");
		} catch (Exception e) {
			assertTrue(e instanceof WebApplicationException);
		}
		...
		try {
			negResponse = resource.update(user, "BAD-UUID", null, entity);
			fail("Should have thrown a WebApplicationException but did not");
		} catch (Exception e) {
			assertTrue(e instanceof WebApplicationException);
		}
		...
		try {
			when(mockRepository.findOne("511846484800016990f86233")).thenReturn(
					null);
			negResponse = resource.update(user, "511846484800016990f86233", null, entity);
			fail("Should have thrown a WebApplicationException but did not");
		} catch (Exception e) {
			assertTrue(e instanceof WebApplicationException);
		}
	...
	public void findSingleEntityByUuid() {
		...
		final T foundEntity = resource.findOne(user, "511846484800016990f86233");
		...
		try {
			when(mockRepository.findOne("511846484800016990f86233")).thenReturn(null);
	...
	public void findAllEntities() {
		...
		final Iterable<T> foundEntityList = resource.findAll(user, null,
	...
	public void deleteEntity() {
		...
		final Response response = resource.delete(user, "511846484800016990f86233");
